import { describe, it, expect, vi } from "vitest";
import React from "react";
import { render, screen, fireEvent, waitFor } from "@testing-library/react";
import { QueryClient, QueryClientProvider } from "@tanstack/react-query";
import { notify } from "../../../lib/feedback";
import { adminApi } from "../../../lib/api/admin.api";

// Mock notify feedback
vi.mock("../../../lib/feedback", () => ({
  notify: {
    success: vi.fn(),
    error: vi.fn(),
    warning: vi.fn(),
    info: vi.fn(),
    loading: vi.fn(),
    dismiss: vi.fn(),
  },
  formatErrorMessage: vi.fn((e: any) => e?.message || "Error"),
}));
import { AdminDashboard } from "../AdminDashboard";
import { UsersPage } from "../UsersPage";
import { ScoringConfigPage } from "../ScoringConfigPage";
import { ScoringQualityPage } from "../ScoringQualityPage";
import { DatabaseMaintenancePage } from "../DatabaseMaintenancePage";
import { AuditLogsPage } from "../AuditLogsPage";

// Mock useAuth
vi.mock("../../../hooks/useAuth", () => ({
  useAuth: () => ({
    user: { id: "admin-1", email: "admin@megs.ph", role: "ADMINISTRATOR" },
    isAuthenticated: true,
  }),
}));

// Mock adminApi
vi.mock("../../../lib/api/admin.api", () => ({
  adminApi: {
    listUsers: vi.fn().mockResolvedValue([
      {
        id: "admin-1",
        email: "admin@megs.ph",
        role: "ADMINISTRATOR",
        accountStatus: "ACTIVE",
        isActive: true,
        createdAt: "2026-08-01T00:00:00Z",
      },
      {
        id: "ta-1",
        email: "recruiter@megs.ph",
        role: "TALENT_ACQUISITION",
        accountStatus: "PENDING",
        invitationStatus: "PENDING",
        isActive: true,
        createdAt: "2026-08-05T00:00:00Z",
      },
    ]),
    inviteTA: vi.fn(),
    resendTAInvitation: vi.fn(),
    cancelTAInvitation: vi.fn(),
    getScoringConfig: vi.fn().mockResolvedValue({
      id: 1,
      version: 1,
      revision: 3,
      scope: "DEFAULT",
      status: "ACTIVE",
      activatedAt: "2026-08-10T00:00:00Z",
      weights: {
        SKILLS: 30,
        EXPERIENCE: 25,
        LOCATION: 15,
        COMPLIANCE: 15,
        EDUCATION_CERTIFICATIONS: 15,
      },
      knnSettings: {
        defaultK: 10,
        maximumK: 50,
        minimumSimilarity: 0.5,
        excludeCurrentlyHired: true,
      },
      matchThreshold: 60,
    }),
    getScoringConfigHistory: vi.fn().mockResolvedValue([]),
    listDatabaseBackups: vi.fn().mockResolvedValue([
      {
        id: "b-1",
        filename: "megs_backup_2026-08-14.enc.gz",
        sizeBytes: 1048576,
        checksumSha256: "abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890",
        status: "SUCCESS",
        backupType: "MANUAL",
        encryptionMethod: "AES-256-GCM",
        tablesIncluded: ["User", "JobPosting"],
        initiatedBy: { id: "admin-1", email: "admin@megs.ph" },
        createdAt: "2026-08-14T00:00:00Z",
      },
    ]),
    triggerDatabaseBackup: vi.fn(),
    renameDatabaseBackup: vi.fn(),
    downloadDatabaseBackup: vi.fn(),
    exportAuditReport: vi.fn(),
    getQualityMetrics: vi.fn().mockResolvedValue({
      totalCalculated: 150,
      averageFitScore: 79.5,
      minFitScore: 35,
      maxFitScore: 98,
      coveragePercentage: 100,
      knnLatencyP95: 38,
      scoreDistribution: {
        "80-100": 70,
        "60-79": 50,
        "40-59": 20,
        "20-39": 10,
        "0-19": 0,
      },
    }),
    listAuditLogs: vi.fn().mockResolvedValue([
      {
        id: 1,
        action: "SCORING_CONFIG_ACTIVATED",
        userId: "admin-1",
        entity: "CandidateScoringConfiguration",
        entityId: "1",
        createdAt: "2026-08-14T00:00:00Z",
      },
    ]),
  },
}));

// Mock tanstack router components
vi.mock("@tanstack/react-router", () => ({
  Link: ({ children, to, ...props }: any) => (
    <a href={to} {...props}>
      {children}
    </a>
  ),
  useNavigate: () => vi.fn(),
  useParams: () => ({}),
}));

describe("Admin Pages Integration & Rendering", () => {
  const createQueryClient = () =>
    new QueryClient({
      defaultOptions: {
        queries: { retry: false },
      },
    });

  const renderWithClient = (ui: React.ReactElement) => {
    const queryClient = createQueryClient();
    return render(
      <QueryClientProvider client={queryClient}>{ui}</QueryClientProvider>
    );
  };

  it("renders AdminDashboard with tiles, charts, and metrics", async () => {
    renderWithClient(<AdminDashboard />);
    expect((await screen.findAllByText("Administration overview")).length).toBeGreaterThan(0);
    expect(await screen.findByText("Registered Accounts")).toBeDefined();
    expect((await screen.findAllByText("Data Backups")).length).toBeGreaterThan(0);
    expect(await screen.findByText("Candidate Matching Weights")).toBeDefined();
  });

  it("renders UsersPage with user list, invitation status badges, and action buttons", async () => {
    renderWithClient(<UsersPage />);
    expect((await screen.findAllByText("User access")).length).toBeGreaterThan(0);
    expect(await screen.findByText("admin@megs.ph")).toBeDefined();
    expect(await screen.findByText("recruiter@megs.ph")).toBeDefined();
    expect(await screen.findByText("PENDING INVITATION")).toBeDefined();
  });

  it("renders ScoringConfigPage with dimension sliders and validation", async () => {
    renderWithClient(<ScoringConfigPage />);
    expect((await screen.findAllByText("Candidate matching settings")).length).toBeGreaterThan(0);
    expect(await screen.findByText("1. Skills & Technical Competencies")).toBeDefined();
    expect(await screen.findByText("Talent Discovery & Match Parameters")).toBeDefined();
    expect(await screen.findByText("Advanced Algorithm Settings")).toBeDefined();
  });

  it("renders ScoringQualityPage with histogram, telemetry, and action buttons", async () => {
    renderWithClient(<ScoringQualityPage />);
    expect((await screen.findAllByText("Candidate matching quality")).length).toBeGreaterThan(0);
    expect(await screen.findByText("Candidate Match Score Distribution")).toBeDefined();
    expect(screen.getByRole("button", { name: /Refresh/i })).toBeDefined();
    expect(screen.getByRole("button", { name: /Configure Weights/i })).toBeDefined();
  });

  it("renders DatabaseMaintenancePage with snapshot history, encryption telemetry, and backup action", async () => {
    renderWithClient(<DatabaseMaintenancePage />);
    expect((await screen.findAllByText("Data backup & recovery")).length).toBeGreaterThan(0);
    expect(await screen.findByText("Total Backups")).toBeDefined();
    expect(await screen.findByText("megs_backup_2026-08-14.enc.gz")).toBeDefined();
    expect(screen.getByRole("button", { name: /Create Backup/i })).toBeDefined();
    expect(screen.getByRole("button", { name: /Upload & Restore/i })).toBeDefined();
    expect(screen.getByRole("button", { name: /^Restore$/i })).toBeDefined();

    // Verify filter toolbar & search
    expect(screen.getByPlaceholderText(/Search backup filename/i)).toBeDefined();
    expect(screen.getByRole("button", { name: /^Completed$/i })).toBeDefined();
    expect(screen.getByRole("button", { name: /^7 Days$/i })).toBeDefined();
  });

  it("renders AuditLogsPage with filterable audit ledger and human-readable labels", async () => {
    renderWithClient(<AuditLogsPage />);
    expect((await screen.findAllByText("Activity log")).length).toBeGreaterThan(0);
    const actions = await screen.findAllByText("Scoring Configuration Activated");
    expect(actions.length).toBeGreaterThan(0);
    const categories = await screen.findAllByText("Configuration");
    expect(categories.length).toBeGreaterThan(0);
  });

  it("handles audit log export errors with notify feedback", async () => {
    const error = new Error("Failed to export report");
    vi.mocked(adminApi.exportAuditReport).mockRejectedValueOnce(error);

    renderWithClient(<AuditLogsPage />);
    expect((await screen.findAllByText("Activity log")).length).toBeGreaterThan(0);

    const exportPdfBtn = await screen.findByRole("button", { name: /Export PDF Report/i });
    fireEvent.click(exportPdfBtn);

    await waitFor(() => {
      expect(notify.error).toHaveBeenCalledWith("Export Failed", error);
    });
  });
});
